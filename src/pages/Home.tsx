...
import { ArrowRight, ArrowUpRight, Scissors, Sparkles, Home as HomeIcon, Clock, MapPin } from 'lucide-react';
...
export default function Home() {
  ...
  const featured = services.filter((s) =>
    ['hair-styling-blow-dry', 'braiding', 'facials', 'massage-therapy', 'hair-colouring', 'pedicure'].includes(s.slug)
  );

  return (
    <>
      {/* Hero — split editorial layout, not the centered template */}
      <section className="relative min-h-screen flex items-end bg-ink-900 overflow-hidden">
        <div className="absolute inset-0">
          <img
            src={IMAGES.salonInterior3}
            alt="Fargo salon interior with modern lighting and round mirrors"
        ...
        <div className="relative w-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pb-16 lg:pb-24 pt-32">
          <div className="grid lg:grid-cols-12 gap-8 items-end">
            <div className="lg:col-span-7">
              <p className="text-xs uppercase tracking-wider-3 text-cream-100/70 mb-6 animate-fade-up">
                Southeast Nigeria — Unisex Salon & Spa
              </p>
              <h1 className="text-4xl sm:text-5xl lg:text-7xl font-display text-cream-50 leading-[1.05] text-balance animate-fade-up" style={{ animationDelay: '0.1s' }}>
                Hair, beauty, and<br />
                wellness — considered,<br />
                not rushed.
              </h1>
              <p className="mt-8 text-lg text-cream-100/80 max-w-md leading-relaxed animate-fade-up" style={{ animationDelay: '0.2s' }}>
                A unisex salon and spa where every appointment is treated as personal time.
            ...
            <div className="lg:col-span-5 lg:pl-8 animate-fade-up" style={{ animationDelay: '0.4s' }}>
              <div className="grid grid-cols-3 gap-4 lg:gap-2">
                <Stat number="20+" label="Services" />
                <Stat number="4" label="Specialists" />
                <Stat number="2" label="Locations" sub="Salon & Home" />
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-20 lg:py-32 bg-cream-50">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
            ...
            <div className="lg:col-span-7">
              <Reveal>
                <p className="text-xs uppercase tracking-wider-3 text-rose-500 mb-4">Our Philosophy</p>
                <h2 className="text-3xl sm:text-4xl lg:text-5xl font-display text-ink-900 leading-[1.15] text-balance">
                  We don't just style hair. We give people back their time.
                </h2>
              </Reveal>
              <Reveal delay={100}>
                <div className="mt-8 space-y-5 text-ink-600 leading-relaxed text-base sm:text-lg">
                  <p>
                    Fargo is built on a simple idea: a salon visit should feel like a break,
                    not a transaction. Whether you're here for a quick trim or a full spa day,
                    the pace is yours.
                  </p>
                  <p>
                    Our team covers the full range — braiding, colouring, cuts, facials, massage,
                    nails, and more. And for days when you can't come in, we bring the salon to your door.
                  </p>
                </div>
              </Reveal>
              <Reveal delay={200}>
      ...
      {/* Signature Services — staggered layout, not a grid */}
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
          <Reveal>
            <SectionHeading
              ...
              subtitle="A curated selection from our full menu. Each one is performed by a specialist who works on it every day."
              light
            />
          </Reveal>
          ...
          <div className="mt-14 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {loading
              ? [...Array(6)].map((_, i) => <div key={i} className="h-72 bg-ink-800 animate-pulse" />)
              : featured.map((service, i) => (
                  <Reveal key={service.id} delay={i * 80} as="article">
                    <Link
                      to={`/services/${service.slug}`}
                      className="group block bg-ink-800 hover:bg-ink-700 transition-colors duration-300 p-7 h-full border border-ink-700"
                    >
                      <div className="flex items-start justify-between mb-6">
                        ...
                        <ArrowUpRight size={18} className="text-ink-500 group-hover:text-rose-400 group-hover:rotate-45 transition-all duration-300" />
                      </div>
                      <h3 className="text-2xl font-display text-cream-50 mb-3 group-hover:text-rose-400 transition-colors">
                        {service.name}
                      </h3>
                      <p className="text-sm text-ink-300 leading-relaxed clamp-3 mb-6">
                        {service.description}
                      </p>
                      <div className="flex items-center justify-between text-xs text-ink-400 pt-4 border-t border-ink-700">
                        <span className="flex items-center gap-1.5">
                          <Clock size={13} /> {formatDuration(service.duration_minutes)}
                        </span>
                        <span className="text-cream-100">{formatPriceRange(service.price_min, service.price_max)}</span>
                      </div>
                    </Link>
                  </Reveal>
                ))}
      ...
      {/* Home Service — full-width banner with image */}
      <section className="relative py-24 lg:py-36 overflow-hidden">
        <div className="absolute inset-0">
          <img
            src={IMAGES.spaMassage2}
            alt="Massage therapy in a calm setting"
          ...
          <div className="max-w-xl">
            <Reveal>
              ...
              <p className="mt-6 text-lg text-cream-100/80 leading-relaxed">
                ...
                by the same specialists you'd see in our chair. Available within our coverage area.
              </p>
              <Link
                to="/home-services"
      ...
      {/* Atmosphere — editorial photo layout */}
      <section className="py-20 lg:py-32 bg-cream-50">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
          <Reveal>
            <SectionHeading
              eyebrow="The Space"
              title="A room designed for slowing down"
              align="center"
            />
          </Reveal>
      ...
      <section className="py-20 lg:py-28 bg-rose-500">
        <div className="max-w-4xl mx-auto px-5 sm:px-8 text-center">
          <Reveal>
            <Sparkles size={28} className="text-cream-50 mx-auto mb-6" />
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-display text-cream-50 leading-[1.15] text-balance">
              Ready when you are.
            </h2>
            <p className="mt-5 text-cream-50/90 text-lg max-w-md mx-auto">
              Book in a few steps. No account needed — just pick a time that works.
            </p>
            <Link
              to="/booking"
      ...
      <section className="py-12 bg-cream-100 border-t border-ink-100">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-3 text-ink-700">
            ...
            <span className="text-sm">Southeast Nigeria — in-salon and at-home service</span>
          </div>
          <div className="flex items-center gap-6 text-sm text-ink-600">
            <span className="flex items-center gap-2"><Clock size={15} /> Tue–Sat 9–7</span>
            <span className="flex items-center gap-2"><Clock size={15} /> Sun 12–6</span>
          </div>
        </div>
      </section>
    </>
...
function Stat({ number, label, sub }: { number: string; label: string; sub?: string }) {
  return (
    <div className="text-center lg:text-left">
      <p className="text-3xl lg:text-4xl font-display text-cream-50">{number}</p>
      <p className="text-xs uppercase tracking-wider-2 text-cream-100/60 mt-1">{label}</p>
      {sub && <p className="text-[10px] text-cream-100/40 mt-0.5">{sub}</p>}
    </div>
  );
}
